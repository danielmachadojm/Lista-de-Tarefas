...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (opcao) {
                ...
                case 4 -> marcarComoConcluida();
                case 5 -> marcarComoPendente(); // nova função, abaixo
                case 6 -> {
                    System.out.println("\n-- ORDENAR TAREFAS --");
                    ...
                    scanner.nextLine();
                    switch (subOpcao) {
                        case 1 -> ordenarPorPrioridade();
                        case 2 -> ordenarPorData();
                        case 3 -> ordenarPorTitulo();
                        case 4 -> ordenarPorStatus();
                        default -> System.out.println("Opção inválida.");
                    }
                }
                case 7 -> exportarTarefas();
    ...
    private static void marcarComoPendente() {
        listarTarefas();
        if (tarefas.isEmpty()) return;
    ...
    private static void exportarTarefas() {
        if (tarefas.isEmpty()) {
    ...
    private static void exportarComoTxt(String nomeArquivo) {
    ...
    private static void exportarComoCsv(String nomeArquivo) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(nomeArquivo))) {
    ...
    private static void exportarComoPdf(String nomeArquivo) {
        try {
            Class<?> clazz = Tarefa.class;

            if (!clazz.isAnnotationPresent(ExportarComoPDF.class)) {
            ...
            document.add(new com.lowagie.text.Paragraph(" ")); // espaço em branco
